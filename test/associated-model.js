/*jslint nomen: true*/
/*global $, _, document, window, console, Backbone, module, test, equal, ok, deepEqual, notEqual, strictEqual*/
...
$(document).ready(function () {
    ...
    if (!window.console) {
        window.console = {};
        $.each(names, function () { window.console[this] = function () {}; });
    }


    Location = Backbone.AssociatedModel.extend({
        defaults: {
            id: -1,
            add1: "",
            add2: null,
    ...
    map2locs = function (ids) {
        ...
        if (_.isArray(ids) && ids.length > 0) {
            if (_.isObject(ids[0])) { return ids; }//dummy logic to check whether array has ids or objects
        } else {
            if (_.isObject(ids)) { return ids; }
        }
    ...
    Project = Backbone.AssociatedModel.extend({
        relations: [
            {
                type: Backbone.Many,
                key: 'locations',
    ...
    Department = Backbone.AssociatedModel.extend({
        relations: [
            {
                type: Backbone.Many,
                key: 'controls',
    ...
    //dept store
    store = new Backbone.Collection([new Department({number: 99, name: "sales"}), new Department({number: 100, name: "admin"})]);
    ...
    test("unset", 3, function () {
        ...
        equal(emp.get('works_for').get('locations'), undefined, "locations should be void");

        emp.unset('works_for.locations');
        equal(emp.get('works_for.locations'), undefined, "locations should be void");
        emp.unset('dependents');
        equal(emp.get('dependents'), undefined, "`dependents` should be unset");
    });
    ...
    test("invalid relations", 4, function () {
        var Em1, em1, Em2, em2, Em3, em3, Owner, owner, House, house;
        Em1 = Backbone.AssociatedModel.extend({
            relations: [
                {
        ...
        Em2 = Backbone.AssociatedModel.extend({
            relations: [
                {
        ...
        try {
            ...
            equal(e2.message === "type attribute must be specified and have the values Backbone.One or Backbone.Many", true);
        }

        Em3 = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'parent',
        ...
        try {
            ...
            equal(e3.message === "specify a relatedModel for Backbone.One type", true);
        }

        Owner = Backbone.OriginalModel.extend();
        ...
        owner = new Owner();
        try {
            house = new House({owner: owner});
        } catch (e4) {
            equal(e4.message === "specify an AssociatedModel for Backbone.One type", true);
        }
    ...
    test("clear", 2, function () {
        emp.clear();
        equal(emp.get('works_for'), undefined, "Deparment should be set to undefined");
        equal(emp.get('dependents'), undefined, "Dependents should be undefined");
    });
    ...
    test("child `nested-change`", 9, function () {
        ...
        emp.get('works_for').on('nested-change', function () {
            ...
            if (params[0] === "controls[0].locations[0]") {
                ok(true, "Fired emp.works_for change:controls[0].locations[0]...");
            }
            if (params[0] === "locations[0]") {
                ok(true, "Fired emp.works_for change:locations[0]...");
            }
            equal(params[2].dummy, true);

        });

        emp.on('nested-change', function () {
            var params = arguments;
            if (params[0] === "works_for.controls[0].locations[0]") {
                ok(true, "Fired emp change:works_for.controls[0].locations[0]...");
            }
            if (params[0] === "works_for.locations[0]") {
                ok(true, "Fired emp change:works_for.locations[0]...");
            }
    ...
    test("Set closure scope correctly - while setting BB Collection & Model instances directly", 5, function () {
        var Location, Group, loc1, loc2, cb, g1, g2;
        Location = Backbone.AssociatedModel.extend({
            defaults: {
                name: "",
                zip: ""
        ...
        Group = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'meetup',
        ...
        loc1 = new Location({name: "CA", zip: "94404", id: 1});
        loc2 = new Location({name: "OH", zip: "92201", id: 2});

        cb = function () {
            ok(true);
        };

        g1 = new Group({name: "AO", meetup: loc1});
        g1.on("change:meetup", cb);

        g2 = new Group({name: "SO", meetup: loc2});
    ...
    test("Fetch collection repeatedly: Issue#47", 3, function () {
        var User, Container, ContainerCollection, f;
    ...
    test("Polymorphic Associate : Issue#23", 4, function () {
        var Models = {}, findPolyMorphicType, cjob, djob, articleComment, statusComment;

        findPolyMorphicType = Models.findPolyMorphicType = function (relation, attributes) {
        ...
        cjob = new Models.Job({organizable_type: 'Company', name: "J1", organizable: {name: "Google"}});
        djob = new Models.Job({organizable_type: 'Department', name: "J2", organizable: {name: "Google Reader"}});

        articleComment = new Models.Comment({
            commentable_type: 'Article',
            name: "c1",
            commentable: {body: "Wonderful post!"}
        });
        statusComment = new Models.Comment({
    ...
    test("Fetch collection with reset: Issue#45", 5, function () {
        var Product, Category, Brand, Vendor, Tag, SearchFacet, counter, SearchResult, searchResult;
        Product = Backbone.AssociatedModel.extend({
            defaults: {
        ...
        Category = Backbone.AssociatedModel.extend();
        Brand = Backbone.AssociatedModel.extend();
        Vendor = Backbone.AssociatedModel.extend();
        Tag = Backbone.AssociatedModel.extend();

        SearchFacet = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.Many,
                    key: 'categories',
        ...
        counter = 1;
        SearchResult = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'searchFacet',
        ...
        searchResult.on("reset:products", function () {
            ok(false, 'searchResult reset:products should not fired.'); //0
        });
        searchResult.on("reset:searchFacet.tags", function () {
    ...
    test("IdAttribute: Issue#80", 2, function () {
        ...
        home = new Container();
        u = new User({_id: 1, name: "Chip Lay"});
        home.set('owner', u);
        u1 = new User({_id: 1, name: "Chip Lay"});
        home.set('owner', u1);
    ...
    test("Cycle save: Issue#51", 3, function () {
        var MyApp, Models, provinceRecord, childrenMinders;
        MyApp = {
            Models: {},
        ...
        Models.Record = Backbone.AssociatedModel.extend({
            ...
            sync: function (method, model, options) {
                ...
                if (method === 'create') {
                    response[model.idAttribute] = (counter += 1); // dummy id
        ...
        provinceRecord = new Models.Record({id: 2, name: 'test1'});
        MyApp.Context.provinceRecords.push(provinceRecord);

        childrenMinders = new Models.ChildrenMinders([
    ...
    test("Polymorphic collection models: Issue#73", 2, function () {
        var Club, club;
        Club = Backbone.AssociatedModel.extend({
            relations: [{
                type: Backbone.Many,
                key: 'members',
    ...
    test("Polymorphic associations + map: Issue#54", 9, function () {
        var Fruit, Banana, Tomato, polymorphic, fruitStore, lazy, Oatmeal,
            aHealthyBowl, aHealthyBowl2, FruitExplosion, bananaExplosion, tomatoExplosion;
        Fruit = Backbone.AssociatedModel.extend();
        Banana = Fruit.extend();
        Tomato = Fruit.extend();

        polymorphic = function (relation, attributes) {
            var key = relation.key + '_type';
        ...
        Oatmeal = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'fruitable',
        ...
        FruitExplosion = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.Many,
                    key: 'fruitable',
        ...
        bananaExplosion = new FruitExplosion({fruitable_type: Banana, fruitable: [3, 4]});
        ...
        tomatoExplosion = new FruitExplosion({fruitable_type: Tomato, fruitable: [3, 4]});
    ...
    test("Issue #28", 2, function () {
        var ItemModel, item;
        ItemModel = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'product',
        ...
        item = new ItemModel({ product: { name: 'johnny' } });
    ...
    test("Issue  #35", 4, function () {
        var FieldInputType, Field, second, job, first;
        FieldInputType = Backbone.AssociatedModel.extend({
            defaults: {
                id: "-1",
                type: "",
                source: "<div/>"
        ...
        Field = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'type',
        ...
        second = new Field({name: 'First Name', type: 'text' });
        job = new Field({name: 'Job', type: 'email'});
    ...
    test("Issue #40", 5, function () {
        var CartItem, Cart, Account, a, c, ci1, ci2, ci3;
        CartItem = Backbone.AssociatedModel.extend({
            defaults: {
                qty: 0
            }
        });

        Cart = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.Many,
                    key: 'items',
        ...
        Account = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'cart',
        ...
        ci1 = new CartItem({qty: 5});
        ci2 = new CartItem({qty: 7});
    ...
    test("Issue #31", 5, function () {
        var Model1, Model2, m2, m1;
        Model1 = Backbone.AssociatedModel.extend({});
        Model2 = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: "model1",
        ...
        m2 = new Model2({id: 1, model1: {id : 2, name: "Name"}, version: "m2.0"});
        m1 = m2.get("model1");
    ...
    test("Issue#67 : Nested set shouldn't destroy references", 2, function () {
        ...
        team = new Team(json1);
        employee1 = team.at(0);
        manager1 = employee1.get('manager');
    ...
    test("Issue #31 nested collection", 2, function () {
        var Node, treeJson, treeModel, nodes0, nodes1;
        Node = Backbone.AssociatedModel.extend({
            defaults: {
                id: null,
                value: "",
        ...
        treeModel = new Node(treeJson);
        nodes0 = treeModel.get("nodes");
        nodes1 = treeModel.get("nodes[0].nodes");
    ...
    test("add multiple refs to the same collection", 6, function () {
        ...
        emp.on('change:works_for.controls[1].locations[1].zip', function () {
            ok(true, "Fired emp > change:works_for.controls[1].locations[1].zip");
        });

        emp.on('change:works_for.controls[1].locations[1]', function () {
    ...
    test("toJSON", 2, function () {
        var json1, rawJson1, json2, rawJson2;
        json1 = emp.get('dependents').toJSON();
        rawJson1 = [
    ...
    test("Backbone.Self", 9, function () {
        ...
        user1 = new User({id: 1, username: 'user1'});
        user2 = new User({id: 2, username: 'user2', friends: [user1]});
        user3 = new User({id: 3, username: 'user3', friends: [user1, {id: 4, username: 'user4'}]});
    ...
    test("Defaults clear", function () {
        ...
        equal(emp.get('works_for').get('type'), undefined);
    ...
    test("`change:attr` and `change` event with options", 3, function () {
        emp.on("change", function (employeeModel) {
            equal(employeeModel.get("fname"), emp.get("fname"));
        });
        emp.on("change:works_for", function (employeeModel, changedWorksFor) {
            equal(employeeModel.get("fname"), emp.get("fname"));
            equal(changedWorksFor, undefined);
        });
        emp.set({
            lname: 'Hanks',
            works_for: undefined
    ...
    test("parents - Issue #39", 3, function () {
        var Nested, Child, Root, root, child;
        Nested = Backbone.AssociatedModel.extend();

        Child = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'nested',
        ...
        Root = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: 'child',
    ...
    test("Many relation's options : parse", 3, function () {
        ...
        Company = Backbone.AssociatedModel.extend({
            url: "/company",
            relations: [
                {
                    type: Backbone.Many,
            ...
            sync: function (method, model, options) {
                ...
                return options.success.call(this, {
                    name: 'c-name',
                    employees: [{fname: "John", lname: "Smith", age: 21, sex: "M"}]
                }, options);
            }
        });
        company = new Company();
    ...
    test("One relation's options passed from parent", 2, function () {
        ...
        Room = Backbone.AssociatedModel.extend({
            url: "/unit",
            relations: [
                {
                    type: Backbone.One,
    ...
    test("`visited` flag results in wrong toJSON output in event callback : issue #3", 3, function () {
        ...
        dependents.reset();
        json = {"fname": "Jane", "lname": "Smith", "sex": "F", "age": 0, "relationship": "C"};
    ...
    test("Saving child of Model in Collection result in improper overwrite on return : issue#17", 2, function () {
        var Child, Parent, Collection, collection, parent;
        Child = Backbone.AssociatedModel.extend({
            defaults: {
                favoriteToy: "",
                color: ""
        ...
        Parent = Backbone.AssociatedModel.extend({
            relations: [
                {
                    type: Backbone.One,
                    key: "child",
